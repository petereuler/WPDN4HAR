...
class WaveletPacketDecomposer(nn.Module):
    ...
    def forward_concat(self, x):
        """
        返回按频带顺序拼接后的张量，减少CPU推理时的Python list开销。
        """
        # 第1级分解
    ...
    def forward(self, x):
        ...
        """
        level_concat = self.forward_concat(x)
        num_bands = 2 ** self.decompose_levels
        return [level_concat[:, i*self.in_channels:(i+1)*self.in_channels] for i in range(num_bands)]
...
class TimeFrequencyMapGenerator(nn.Module):
    ...
    def forward(self, frequency_bands):
        if isinstance(frequency_bands, torch.Tensor):
            batch_size, total_channels, time_bins = frequency_bands.shape
            num_bands = total_channels // self.in_channels
            time_freq_map = frequency_bands.view(batch_size, num_bands, self.in_channels, time_bins)
            return time_freq_map.permute(0, 2, 1, 3)

        stacked_bands = torch.stack(frequency_bands, dim=1)
        return stacked_bands.permute(0, 2, 1, 3)
...
class BandLocalSparseConv1d(nn.Module):
    """
    使用局部频带稀疏连接的1x1 Conv1d，在每个时间步上只允许相邻频带之间混合。

    输入需要按 [band0_channels, band1_channels, ...] 的顺序拼接。
    """
    def __init__(self, num_bands, band_kernel_size=3):
        super().__init__()
        if num_bands < 1:
            raise ValueError("num_bands must be >= 1")

        effective_kernel = min(band_kernel_size, num_bands)
        if effective_kernel % 2 == 0:
            effective_kernel = max(1, effective_kernel - 1)

        self.num_bands = num_bands
        self.band_kernel_size = effective_kernel

        weight = torch.zeros(num_bands, num_bands, 1)
        mask = torch.zeros_like(weight)
        radius = effective_kernel // 2

        for out_band in range(num_bands):
            left = max(0, out_band - radius)
            right = min(num_bands, out_band + radius + 1)
            mask[out_band, left:right, 0] = 1.0
            weight[out_band, out_band, 0] = 1.0

        self.weight = nn.Parameter(weight)
        self.register_buffer("mask", mask, persistent=False)

    def forward(self, x, feature_groups):
        batch_size, total_channels, time_steps = x.shape
        expected_channels = feature_groups * self.num_bands
        if not torch.jit.is_tracing() and total_channels != expected_channels:
            raise ValueError(
                f"Expected {expected_channels} channels for {feature_groups} feature groups and "
                f"{self.num_bands} bands, but got {total_channels}"
            )

        x = x.reshape(batch_size, self.num_bands, feature_groups, time_steps)
        x = x.permute(0, 2, 1, 3).reshape(batch_size * feature_groups, self.num_bands, time_steps)
        x = F.conv1d(x, self.weight * self.mask)
        x = x.reshape(batch_size, feature_groups, self.num_bands, time_steps)
        x = x.permute(0, 2, 1, 3).reshape(batch_size, total_channels, time_steps)
        return x
...
class LightweightWaveletPacketCNN(nn.Module):
    def __init__(self, in_channels=6, num_classes=6, input_length=128, kernel_size=4,
        ...
        if use_traditional_wavelet:
            ...
            self.feature_groups = in_channels
            self.sequence_num_bins = freq_bins
            sequence_shape = (in_channels * freq_bins, time_bins)
        elif use_parallel:
            ...
            num_frequency_bands = 2 ** decompose_levels
            self.feature_groups = in_channels * num_parallel_groups
            self.sequence_num_bins = num_frequency_bands
            sequence_shape = (in_channels * num_parallel_groups * num_frequency_bands, input_length // (2 ** decompose_levels))
        else:
    ...
    def _forward_impl(self, x):
        if self.use_traditional_wavelet:
            ...
            batch_size, channels, freq_bins, time_bins = time_freq_map.shape
    ...
    def forward(self, x):
        optimized_model = self._cpu_inference_model
        if (
            optimized_model is not None
    ...
    def enable_cpu_fast_inference(self, example_input=None):
        if next(self.parameters()).device.type != "cpu":
            raise RuntimeError("CPU fast inference only supports CPU models")
        ...
        if example_input is None:
            example_input = torch.randn(1, self.in_channels, self.input_length, device="cpu")
        else:
        ...
        object.__setattr__(self, "_cpu_inference_model", optimized_model)

        return self

    def enable_cpu_fast_classifier(self, example_input=None):
        return self.enable_cpu_fast_inference(example_input=example_input)

    def disable_cpu_fast_inference(self):
        object.__setattr__(self, "_cpu_inference_model", None)
        return self
